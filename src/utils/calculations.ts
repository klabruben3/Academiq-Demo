import { differenceInDays, parseISO, isAfter, isBefore, format, startOfWeek, addWeeks } from 'date-fns';
import type { Module, AssessmentComponent, TimelineEvent, Priority, PriorityItem, WorkloadDataPoint } from '../types';
...
export function getAssessmentPriority(daysUntil: number, weight: number): Priority {
...
export function buildTimeline(modules: Module[], scores: Record<string, number>): TimelineEvent[] {
  ...
  for (const mod of modules) {
    for (const assessment of mod.assessments) {
      ...
      const completed = assessment.id in scores || (assessment.completed ?? false);
...
export function calculateParticipationMark(mod: Module, scores: Record<string, number>): number {
  const formula = mod.participationFormula;
  let total = 0;

  for (const comp of formula.components) {
    const assessments = mod.assessments.filter(a => a.category === comp.componentId.replace(`${mod.id}-`, ''));

    if (assessments.length === 0) {
      // Direct single assessment
      const assessment = mod.assessments.find(a => a.id === comp.componentId);
      if (assessment && assessment.id in scores) {
        const pct = (scores[assessment.id] / assessment.maxScore) * 100;
        total += pct * (comp.weight / 100);
      }
    } else {
      // Multiple assessments in category
      let marks: number[] = assessments
        .filter(a => a.id in scores)
        .map(a => (scores[a.id] / a.maxScore) * 100);

      if (comp.dropLowest && marks.length > comp.dropLowest) {
        marks.sort((a, b) => a - b);
        marks = marks.slice(comp.dropLowest);
      }
...
export function getUpcomingEvents(events: TimelineEvent[], days: number): TimelineEvent[] {
